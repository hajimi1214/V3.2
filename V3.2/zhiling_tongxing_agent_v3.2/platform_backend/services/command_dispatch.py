...
def load_command_targets() -> Dict[str, Any]:
    default_config: Dict[str, Any] = {
        ...
        "targets": [
            ...
            {"name": "上位机控制端C", "host": "192.168.1.104", "port": 9000},
        ],
    }
    ...
    for idx in (1, 2, 3):
        ...
        if env_host:
            env_targets.append({"name": f"上位机控制端{idx}", "host": env_host, "port": coerce_int(env_port, 9000)})
    if env_targets:
    ...
    for i, item in enumerate(default_config.get("targets", []), start=1):
        ...
        host = host.strip()
        if host:
            normalized.append({"name": name, "host": host, "port": int(port), "ack_timeout_seconds": float(default_config.get("ack_timeout_seconds", 0.35) or 0.35)})
    default_config["protocol"] = "tcp_plain"
    default_config["targets"] = normalized[:3]
...
def send_control_signal_to_target(target: Dict[str, Any], timeout: float) -> Dict[str, Any]:
    ...
    message = bytes.fromhex("4f4b0a").decode("utf-8")
    started = time.time()
    try:
