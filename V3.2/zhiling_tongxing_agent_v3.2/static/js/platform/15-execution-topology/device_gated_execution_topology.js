...
const EXECUTION_TOPOLOGY_BUILD = 'V3.2.0 Build 20260518';
...
function applyExecutionStepFlowLayout(){
  ...
  const width = Math.max(1120, vp.clientWidth - 16);
  const height = Math.max(620, Math.min(760, Math.max(vp.clientHeight + 140, 620)));
  vp.style.setProperty('--workflow-canvas-w', `${width}px`);
  vp.style.setProperty('--workflow-canvas-h', `${height}px`);
  const positions = {
    state_capture: {left: Math.round(width * .075), top: 48, width: 290},
    context_model: {left: Math.round(width * .56), top: 48, width: 350},
    vision_quality: {left: Math.round(width * .10), top: 190, width: 235},
    localization_3d: {left: Math.round(width * .36), top: 190, width: 235},
    placement_plan: {left: Math.round(width * .62), top: 190, width: 235},
    hardware_interlock: {left: Math.round(width * .10), top: 330, width: 235},
    safety_strategy: {left: Math.round(width * .36), top: 330, width: 235},
    execution_constraint: {left: Math.round(width * .62), top: 330, width: 235},
    fusion_arbitration: {left: Math.round(width * .22), top: 505, width: 340},
    control_task: {left: Math.round(width * .53), top: 505, width: 280},
    archive: {left: Math.round(width * .76), top: 505, width: 250}
  };
...
window.addEventListener('click', (e) => {
  ...
  if(action === 'mode-manual' || action === 'clear-canvas'){
    ...
    setExecutionFlowModeIndicator('manual');
    if(!isDeviceConnected?.()) clearDecisionTreeForNoDevice();
