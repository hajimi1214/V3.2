...
const MODULE_DETAIL_META = {
  ...
  'agent-review': {
    ...
    bullets:['显示输入摘要、命中规则、证据理由','展示工具调用和处理建议','同步显示Agent正在执行的工作步骤','用于比赛讲解Agent可解释性']
  },
  'state-snapshot': {
  ...
  trace: {
    ...
    bullets:['记录Agent输入、状态和输出','记录风险、耗时和工具调用','用于复盘异常和比赛讲解','可进入报告归档打印']
  },
  archive: {
    title:'报告归档摘要', tag:'ARCHIVE REPORT',
    desc:'归档本轮决策的证据、Trace、权限矩阵和任务结果。',
    bullets:['日志打印和浏览器PDF导出','固化设备编号与任务编号','支持成功/降级/人工接管统计','用于赛后复盘和演示材料']
  },
  alarm: {
...
function buildModuleDetailView(targetId){
  const meta = MODULE_DETAIL_META[targetId] || {title: moduleTitleMap[targetId] || '平台模块', tag:'PLATFORM MODULE', desc:'平台功能模块详情。', bullets:['模块已接入首页点击放大','支持从左侧导航进入','可用于比赛讲解和演示','后续可对接真实接口']};
  const wrap = document.createElement('div');
  wrap.className = 'module-detail-view';
  ...
  wrap.innerHTML = `
    ...
    <section class="module-detail-layout">
      ...
      <aside class="module-detail-side">
  ...
  if(targetId === 'tree-panel'){
    ...
    lifecycle.innerHTML = `<h3>控制任务生成链路</h3><div class="control-lifecycle"><span>融合仲裁</span><i></i><span>权限矩阵</span><i></i><span>轨迹生成</span><i></i><span>设备握手</span><i></i><span>等待下发</span></div><p>未接入硬件设备时，平台只生成预览任务，不允许真实下发。</p></section>`;
    ...
    agentExplain.className = 'agent-explain-panel';
    agentExplain.innerHTML = `<h3>当前审理解释</h3><p>审理台会随SSE事件同步变化：哪个Agent正在决策，就展示哪个Agent的输入、规则、证据、工具调用和工作步骤。</p><div class="agent-role-grid compact">${agentRoleCardsHtml()}</div>`;
