...
if(typeof MODULE_DETAIL_META !== 'undefined'){
  MODULE_DETAIL_META['model-import'] = {
    ...
    bullets:['支持本地模型文件、配置文件和示例模型登记','导入后可注册到Agent组件库','每个Agent显示来源、版本、能力和状态','用于解释Agent不是凭空出现，而是由模型仓库/规则模板/工具链注册而来']
  };
}
...
function buildAgentModelImportView(){
  ...
  </article>`).join('') : '<div class="empty-import-state"><b>暂无Agent模型</b><p>请上传模型/配置/代码文件，或导入示例模型，让评委能看到Agent来源。</p></div>';
...
document.addEventListener('click', (e) => {
  ...
  if(action === 'import-sample-agent-models'){
    ...
    openModuleOverlay('model-import', 'Agent模型导入 / 来源登记');
    showSystemNotice('示例模型已导入', '已注册工业规范治理、提示词优化组件、协同执行组件和安全策略模型Agent，可从组件库加入画布。', '知道了');
  }
