...
try {
  ...
  Object.assign(FLOW_NODE_META.control_task, {name:'控制任务生成模块', layer:'控制模块', role:'非Agent模块：接收仲裁结果，生成控制任务JSON、轨迹序列和权限校验结果', icon:'⚙', status:'PASS', output:'控制任务JSON已生成'});
  ...
  if (AGENT_ROLE_DETAILS[10]) {
    ...
    AGENT_ROLE_DETAILS[10].role = '不是Agent，负责把仲裁结果转换成控制任务JSON和轨迹序列';
  ...
  if (AGENT_ROLE_DETAILS[11]) {
    ...
    AGENT_ROLE_DETAILS[11].role = '不是Agent，负责固化决策Trace、证据链、设备参数和报告打印模板';
...
function resetDefaultFlowCanvas(){
  ...
  applyArbitrationMode();
  redrawFlowCanvas();
  setCanvasHint('已恢复默认多Agent协同流程。点击“一键执行默认配置”时，会像真实决策一样逐步生成节点，不会一次性展示全部流程。');
}
