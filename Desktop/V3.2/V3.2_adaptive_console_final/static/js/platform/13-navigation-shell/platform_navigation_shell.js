...
const PLATFORM_ROUTE_META = {
  ...
  archive:{title:'Trace与报告归档', tag:'TRACE & ARCHIVE'},
...
function zltxReadPersistedDecisionSession(){
  try{ return JSON.parse(localStorage.getItem('zltx_decision_session_v33') || 'null'); }
  catch(err){ return null; }
}
function zltxHasPersistedDecisionSession(){
  const session = zltxReadPersistedDecisionSession();
  if(!session || typeof session !== 'object') return false;
  const revealed = Array.isArray(session.revealed_nodes) ? session.revealed_nodes.length : 0;
  const nodeStatus = session.node_status && typeof session.node_status === 'object' ? Object.keys(session.node_status).length : 0;
...
function zltxRestoreDecisionTreeIfAvailable(){
  if(!zltxHasPersistedDecisionSession()) return false;
  ...
  try{ restoreDecisionSession?.(); }catch(err){}
  try{ redrawFlowCanvas?.(); }catch(err){}
  return true;
}
...
function routeToModule(targetId='workbench', title){
  ...
  if(targetId === 'tree-panel'){
    ...
    route.setAttribute('aria-hidden','true');
    try{
      if(!zltxRestoreDecisionTreeIfAvailable()){
    ...
    }catch(e){ console.warn('V3.2决策树路由恢复失败', e); }
    return;
  }
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    ...
    if(!zltxRestoreDecisionTreeIfAvailable()){
      // 首次进入平台时保持“未连接设备 + 空白决策树”，不自动展开示例拓扑。
      try{
    ...
    document.body.classList.remove('boot-empty-tree');
  }, 480);
});
...
if(routeToModuleBeforeEmptyFlow){
  routeToModule = function(targetId='workbench', title){
    if(targetId === 'tree-panel'){
      ...
      route?.classList.remove('open');
      route?.setAttribute('aria-hidden','true');
      if(typeof window.__zltxHasPersistedDecisionSession === 'function' && window.__zltxHasPersistedDecisionSession()){
        document.body.classList.remove('empty-workflow-flow','no-device-execution-flow');
...
const NAVIGATION_SHELL_BUILD = 'V3.2';
