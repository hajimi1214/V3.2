function openModuleOverlay(targetId, title){
  ...
  if(target && targetId !== 'workbench') {
    ...
    content.innerHTML = `<div class="virtual-module"><div><h3>${title || moduleTitleMap[targetId] || '平台模块'}</h3><p>该模块承载工业智能决策平台的业务能力入口。</p><p>支持设备参数、模型管理、权限配置和生产任务队列接入。</p><div class="fake-grid"><span>设备参数</span><span>Agent策略</span><span>系统日志</span></div></div></div>`;
  }
  overlay.classList.add('open');
  overlay.setAttribute('aria-hidden','false');
}
...
function buildControlTaskFocusView(){
  const wrap = document.createElement('div');
  wrap.className = 'control-focus-view refined';
  const connected = isDeviceConnected();
  const code = $('taskJson')?.textContent || '{}';
  const actionType = connected ? 'PICK_AND_PLACE' : 'WAIT_DEVICE';
  const mode = connected ? 'LIMITED / SAFETY_FIRST' : 'LOCKED';
  const dispatch = connected ? '安全仲裁通过后自动进入上位机下发链路' : '无设备连接，禁止启动决策和指令下发';
  wrap.innerHTML = `
    <section class="panel control-focus-panel">
      <div class="panel-head compact"><h3>控制任务中心 / 工业指令预览</h3><span>COMMAND JSON</span></div>
      <div class="control-command-layout">
        <pre class="large-task-code">${escapeHtml(code)}</pre>
        <aside class="control-command-side">
          <h4>任务解读</h4>
          <div class="control-note-grid refined">
            <p><b>动作类型</b><span>${actionType}</span></p>
            <p><b>安全模式</b><span>${mode}</span></p>
            <p><b>来源Agent</b><span>控制任务生成Agent</span></p>
            <p><b>下发状态</b><span>${dispatch}</span></p>
            <p><b>关键前置</b><span>CRC、急停、安全门、权限矩阵</span></p>
            <p><b>回退策略</b><span>SAFE_STOP_THEN_WAIT_OPERATOR</span></p>
          </div>
          <section class="control-protocol-card">
            <h5>工业下发约束</h5>
            <ul>
              <li>仅在安全仲裁完成后自动下发，异常状态立即阻断。</li>
              <li>任务含ACK超时、重试次数与受限执行策略。</li>
              <li>控制任务会写回Trace与报告归档模块。</li>
            </ul>
          </section>
        </aside>
      </div>
...
function buildArchiveFocusView(){
  const wrap = document.createElement('div');
  wrap.className = 'archive-focus-view refined';
  wrap.innerHTML = `
    <section class="panel archive-focus-panel">
      <div class="panel-head compact"><h3>Trace与报告归档 / 审计复盘</h3><span>ARCHIVE</span></div>
      <div class="archive-focus-actions">
        <button data-action="show-report-log">查看完整归档</button>
        <button data-action="print-archive-pdf">打印 / 另存为PDF</button>
      </div>
      <div class="archive-focus-metrics">
        <p><span>Trace日志</span><b>12</b></p>
        <p><span>证据链</span><b>15</b></p>
        <p><span>工具调用</span><b>7</b></p>
        <p><span>风控结论</span><b>LIMITED</b></p>
      </div>
      <div class="archive-report-preview">${buildArchiveReportHtml()}</div>
    </section>`;
  return wrap;
}
...
function buildArchiveReportHtml(){
  ...
  const taskCode = $('taskJson')?.textContent?.trim() || '{}';
  return `<div class="report-sheet refined-report-sheet">
    <h1>智领铜行--人工智能机械臂多Agent智能决策平台</h1>
    <h2>任务归档日志 / REP-20260509-00187</h2>
    <p class="report-lead">本报告用于记录本轮抓放任务从状态帧采集、多Agent审查、安全仲裁、控制任务生成到归档复盘的完整证据链。</p>
    ...
    <table><tbody>
      ...
      <tr><th>设备编号</th><td>${device && device.connected ? 'DEVICE-001 / 已连接' : '未接入硬件设备'}</td><th>控制器</th><td>${escapeHtml(device?.controller || 'PLC-A03')}</td></tr>
      <tr><th>风险等级</th><td>MEDIUM</td><th>安全模式</th><td>LIMITED</td></tr>
      <tr><th>执行结果</th><td>安全校验通过</td><th>人工确认</th><td>自动下发已受安全仲裁约束</td></tr>
    </tbody></table>

    <h3>二、审查摘要</h3>
    <div class="report-summary-grid">
      <p><span>通过Agent</span><b>4</b><em>视觉 / 定位 / 规划 / 融合</em></p>
      <p><span>受限Agent</span><b>1</b><em>硬件联锁</em></p>
      <p><span>风险Agent</span><b>1</b><em>执行约束</em></p>
      <p><span>最终动作</span><b>PICK_AND_PLACE_LIMITED</b><em>受限安全执行</em></p>
    </div>

    <h3>三、多Agent审理日志</h3>
    <ol class="report-log-list expanded">
      <li>状态帧采集Agent：采集双目相机、AGV视觉、PLC联锁状态帧，CRC校验通过。</li>
      <li>任务上下文建模Agent：识别铜箔抓取工况，拆解看、识、抓、放任务链。</li>
      <li>视觉质量Agent：YOLO铜箔识别置信度0.98，遮挡率3.2%，视觉质量 PASS。</li>
      <li>三维定位Agent：SGBM深度估计RMSE 0.21mm，抓取点位姿可达，定位精度 PASS。</li>
      <li>放置规划Agent：生成第3候选路径，路径可行率92%，碰撞检测 PASS。</li>
      <li>硬件联锁Agent：安全门存在延时确认，触发 LIMITED 降级策略。</li>
      <li>安全策略Agent：综合风险 MEDIUM，允许进入安全优先仲裁。</li>
      <li>执行约束Agent：执行约束核验通过，动作满足标准执行条件。</li>
      <li>融合仲裁Agent：执行 SAFETY_FIRST，输出受限执行裁决。</li>
      <li>控制任务生成Agent：生成含前置校验、轨迹序列、ACK策略与回退策略的控制任务。</li>
      <li>报告归档Agent：固化证据链15条、工具调用7项、Trace日志12条。</li>
    </ol>

    <h3>四、关键证据与风控依据</h3>
    <table class="report-evidence-table"><tbody>
      <tr><th>视觉证据</th><td>OBJECT_DETECT_0981 / RESTORE_QUALITY_094</td><th>定位证据</th><td>POINT_CLOUD_0891 / DEPTH_MAP_2452</td></tr>
      <tr><th>联锁证据</th><td>PLC_IO_BITS / SAFETY_DELAY_WARN</td><th>执行证据</th><td>TORQUE_J2_WARN / SPEED_SCALE_07</td></tr>
      <tr><th>权限矩阵</th><td>ALLOW</td><th>仲裁策略</th><td>SAFETY_FIRST / AUTO_AFTER_ARBITRATION</td></tr>
    </tbody></table>

    <h3>五、控制任务摘要</h3>
    <pre class="report-code-block">${escapeHtml(taskCode)}</pre>

    <h3>六、归档结论</h3>
    <div class="report-conclusion">
      <p>本次任务允许在 LIMITED 安全模式下执行；平台已把风险控制前移至指令下发前。</p>
      <p>建议后续重点复核：安全门延时确认逻辑、J2扭矩阈值与设备心跳稳定性。</p>
      <p>归档状态：Trace、证据链、权限矩阵与控制任务摘要均已固化，可用于答辩展示和赛后复盘。</p>
    </div>
  </div>`;
}
