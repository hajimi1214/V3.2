...
function formatDispatchResults(results){
  ...
  return results.map(item => {
    const status = item.ok ? '已发送' : '发送失败';
    const elapsed = Number.isFinite(Number(item.elapsed_ms))
      ? `${item.elapsed_ms}ms`
      : '待确认';
...
/**
 * 判断多Agent仲裁是否已经完成
 * 用于修复“未经过Agent仲裁也能直接下发指令”的问题
 */
function isAgentDecisionReady() {
  const agentState = document.getElementById("agentBus")?.textContent?.trim();
  return agentState === "完成" || agentState === "DONE";
}
...
async function dispatchIndustrialCommand(){
  if(!isDeviceConnected?.()){
    ...
    showSystemNotice?.(
      '设备状态未就绪',
      '请先进入「系统配置」完成设备接入，再执行指令下发。',
      '进入系统配置',
      () => routeToModule?.('config', '系统配置')
    );
  ...
  try{
    /**
     * 警告Bug修复核心代码：
     * 如果Agent仲裁未完成，禁止直接绕过决策流程下发控制指令
     */
    if (!isAgentDecisionReady()) {
    ...
    if (!response.ok || result.ok === false) {
      throw new Error(result.message || '上位机控制链路未完成指令发送');
    }
    ...
    showSystemNotice?.(
      '决策控制指令已下发',
      formatDispatchResults(result.results),
      '知道了'
    );

    appendLog?.(
      '决策控制指令已下发：' + formatDispatchResults(result.results).replace(/\n/g, '；'),
      new Date().toLocaleTimeString('zh-CN',{hour12:false})
    );
    ...
    showSystemNotice?.(
      '指令下发失败',
      '控制链路发送未完成，请检查接收端、网络连接与控制端配置。',
      '知道了'
    );

    appendLog?.(
      '决策控制指令下发失败：' + (err?.message || '控制链路异常'),
      new Date().toLocaleTimeString('zh-CN',{hour12:false})
    );
...
const updateRunGateBeforeCommandDispatch =
  typeof updateRunGate === 'function'
    ? updateRunGate
    : null;
...
function isWorkflowConfiguredForDispatch(){
  try{
    return Boolean(
      ...
      localStorage.getItem('agent_workflow_config')
    );
  }catch(e){
...
const startCanvasDecisionBeforeCommandDispatch =
  typeof startCanvasDecision === 'function'
    ? startCanvasDecision
    : null;
...
const startDecisionBeforeCommandDispatch =
  typeof startDecision === 'function'
    ? startDecision
    : null;
...
function applyPlatformVersionLabelsFromCommandDispatch(){
  ...
  document.querySelectorAll('.brand-ver').forEach(el => {
    el.textContent = COMMAND_DISPATCH_VERSION;
  });

  const version = document.querySelector('.platform-card .pc-row:last-child strong');
  if(version){
    version.textContent = COMMAND_DISPATCH_BUILD;
  }
...
const applyExecutionTopologyLabelsBeforeCommandDispatch =
  typeof applyPlatformVersionLabelsFromExecutionTopology === 'function'
...
if(applyExecutionTopologyLabelsBeforeCommandDispatch){
  applyPlatformVersionLabelsFromExecutionTopology = function(){
    const ret =
      applyExecutionTopologyLabelsBeforeCommandDispatch.apply(this, arguments);
