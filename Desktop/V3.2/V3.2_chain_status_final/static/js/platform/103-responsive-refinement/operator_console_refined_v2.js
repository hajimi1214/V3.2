 ...
 * 3) 决策安全通过后自动调用控制下发接口；顶部仅展示安全决策链路状态，通信确认保留在日志中。
 */
(function operatorConsoleRefinedV2(){
  ...
  async function autoDispatchAfterSafeDecision(){
    ...
    setDispatchChip('success','安全决策链路：正常');
    ...
    try {
      ...
      setDispatchChip('success','安全决策链路：正常');
      appendStatusLog('控制任务下发记录：上位机控制端已确认接收。');
      ...
      setDispatchChip('success','安全决策链路：正常');
      appendStatusLog('控制任务下发记录：当前未获得上位机接收确认。');
